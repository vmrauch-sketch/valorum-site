...
const AdvancedStrategies = () => {
  ...
  const handleServiceClick = (serviceTitle: string) => {
    ...
    const message = encodeURIComponent(`Olá! Vim do site da Valorum. Gostaria de mais informações sobre ${serviceTitle}`);
    ...
    if (isDevelopment) {
      ...
      navigator.clipboard.writeText(url).then(() => {
        alert(`Link do WhatsApp copiado!\n\nNúmero: 11 95958 6722\nMensagem: "Olá! Vim do site da Valorum. Gostaria de mais informações sobre ${serviceTitle}"\n\nLink copiado para área de transferência.`);
      }).catch(() => {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div ref={headerRef} className={`text-center mb-16 transform transition-all duration-1000 ease-out ${headerVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed mt-8">
              PR² - Planejamento Patrimonial Real e Renda<br />
              Soluções além do planejamento financeiro.
            </p>
          </div>
